...
*它自带一整套严谨护甲：八道质量闸、强制性的交付前审计，以及一次模拟专家评审。*

![工作流](https://img.shields.io/badge/工作流-9%20步-2ea44f)
![质量闸](https://img.shields.io/badge/质量闸-8-blue)
...
| 交付 | "搞定！" | **8 道闸 + 强制审计（0 FAIL）后方可称"完成"** |
...
> *"互联网迅速普及，而使用互联网的劳动者收入明显高于不使用的人。经济学能解释为什么吗？"*
...
| **G8 · 依据** | 每个函数形式/假设/参数限制，是否都有来源**且**有一句"如何支撑"？依据表是否完整？ | 回到 S3/S4；补上支撑，或删掉无依据项 |

### 不可妥协的全局规则

- **禁止伪造引用。** 每条文献都在本次会话中线上核证；无法核实的一律挡在论文之外。
- **基于证据的建模。** 每个函数形式、假设与参数限制都需要一个来源**以及**一句说明它*如何*支撑本模型——单纯引用不算证据。
...
| `scripts/audit_model.py` | **交付前审计（G7 + G8 闸）：** 符号表完整性、参数一致性、修订后的数值回归、交付物卫生、论断安全性、**依据核查**（`--evidence`） |
| `scripts/selftest.py` | **57 条回归用例**，守护解析器/求解器/评审器/审计器——*首次使用前与任何改动后都要运行* |
...
- ✋ **追逐顶刊原创性。** 它瞄准的是*及格*，不是*卓越*。严肃的理论论文，请使用重量级的 `pAI-Econ-claude-main` 工作流。
- ✋ **跑你的回归。** 不做微观数据、不做估计。因果识别请用 `causal-infer-master`。
